...
<body class="bg-[#f4f6f9] min-h-screen font-sans">
        ...
        <div class="container mx-auto flex justify-between items-center px-6 py-3">
            <div class="flex items-center space-x-4">
                ...
                <div class="hidden md:flex space-x-1 text-xs font-medium pt-1">
                    <span class="px-3 py-1 text-gray-300 hover:text-white cursor-pointer transition">Inicio</span>
                    <span class="px-3 py-1 bg-[#003561] rounded text-white font-semibold">Seguimiento & Registro</span>
                </div>
            </div>
            ...
            <div class="flex items-center space-x-4 text-xs">
                ...
                <form action="{{ route('logout') }}" method="POST" class="inline m-0">
                    @csrf
                    <button type="submit" class="flex items-center space-x-1 bg-[#003561] hover:bg-red-700 px-3 py-1.5 rounded font-medium transition shadow-sm">
                        <span>Salir</span>
    ...
    <main class="container mx-auto my-8 max-w-6xl px-4">
        <div class="mb-6">
            <h2 class="text-2xl font-light text-gray-700">Seguimiento <span class="text-sm text-gray-400 font-normal">/ de materias & Registro de Incidentes</span></h2>
            <div class="bg-white border p-3 rounded mt-2 text-center text-sm font-semibold text-gray-600 bg-gray-50 shadow-sm uppercase tracking-wider">
                    ...
                    @forelse($incidentes as $incidente)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="p-3">
                                ...
                                <span class="text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded border border-blue-200">{{ $incidente->tipo_incidente }}</span>
                            </td>
                            ...
                            <td class="p-3">
                                @if($incidente->estado == 'Pendiente')
                                    <span class="bg-red-50 text-red-700 border border-red-200 px-2 py-0.5 rounded text-xs font-semibold">Pendiente</span>
                                @elseif($incidente->estado == 'En proceso')
                                    <span class="bg-yellow-50 text-yellow-700 border border-yellow-200 px-2 py-0.5 rounded text-xs font-semibold">En Proceso</span>
                                @else
                                    <span class="bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-semibold">Resuelto</span>
                                @endif
                            </td>
                            <td class="p-3">
                                <form action="{{ route('incidentes.updateEstado', $incidente->id) }}" method="POST" class="flex items-center gap-2 justify-center">
                                    @csrf
                                    @method('PUT')
                                    <input type="text" name="responsable_asignado" value="{{ $incidente->responsable_asignado }}" placeholder="Asignar a..." class="border rounded p-1 text-xs w-28 outline-none focus:border-blue-500">
                                    <select name="estado" class="border rounded p-1 text-xs bg-white outline-none focus:border-blue-500">
                                    ...
                                    <button type="submit" class="bg-[#004a87] hover:bg-blue-700 text-white text-xs px-2.5 py-1.5 rounded transition">Guardar</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="p-8 text-center text-gray-400">No hay registros de emergencias actualmente.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
